...
class CoziItem(BaseModel):
    ...
    def handle_id_field(cls, values):
        if isinstance(values, dict) and "id" in values and not values.get("itemId"):
...
class CoziList(BaseModel):
    ...
    def parse_items(cls, v):
        if isinstance(v, list):
            return [
                CoziItem.model_validate(item) if isinstance(item, dict) else item
                for item in v
            ]
        return v
    ...
    @classmethod
    def handle_id_field(cls, values):
        if isinstance(values, dict) and "id" in values and not values.get("listId"):
...
class CoziAppointment(BaseModel):
    ...
    def extract_item_details(cls, values):
        """Flatten nested itemDetails fields and fall back to descriptionShort."""
    ...
    def to_api_delete_format(self) -> Dict[str, Any]:
        ...
        return {"itemType": "appointment", "delete": {"id": self.id}}
